...
<template>
  <div v-if="mixinState.mSelectMode === 'one' && state.type === 'image'" class="box">
    <Divider plain orientation="left">
      <h4>{{ $t('picture_filter') }}</h4>
    </Divider>
    <div>
      <div class="filter-item has-params" v-for="item in [...state.paramsFilters]" :key="item.type">
...
import { uiType, paramsFilters } from '@/config/constants/filter';
...
const changeFiltersByParams = (type) => {
  const activeObject = canvasEditor.canvas.getActiveObjects()[0];
  const filtersAll = [...state.paramsFilters];
